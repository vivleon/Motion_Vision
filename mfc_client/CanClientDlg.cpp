    ...
    try {
        ...
        if (m_camTop.IsGrabbing()) {
            if (m_camTop.RetrieveResult(0, grabTop, TimeoutHandling_Return) && grabTop->GrabSucceeded()) {
                ConvertPylonBufferToMat(m_pylonImage, grabTop, currentMatTop, _T("TOP"));
        ...
        if (m_camSide.IsGrabbing()) {
            if (m_camSide.RetrieveResult(0, grabSide, TimeoutHandling_Return) && grabSide->GrabSucceeded()) {
                ConvertPylonBufferToMat(m_pylonImage, grabSide, currentMatSide, _T("SIDE"));
...
// --- Motion Detection ---
void CCanClientDlg::ConvertPylonBufferToMat(CPylonImage& pylonImg,
    CGrabResultPtr& grabResult, cv::Mat& outMat, const CString& role)
{
    try {
        ...
        // (선택) 모션용 반전
        BOOL flipH = (role == _T("TOP")) ? m_bTopFlipH : m_bSideFlipH;
        BOOL flipV = (role == _T("TOP")) ? m_bTopFlipV : m_bSideFlipV;
        if (flipH || flipV) {
            int code = (flipH && flipV) ? -1 : (flipH ? 1 : 0);
            cv::flip(bgrFrame, bgrFrame, code);
        }
    ...
    catch (...) { OutputDebugString(L"[ERROR] ConvertPylonBufferToMat 실패"); }
}
...
void CCanClientDlg::DrawImageBufferToCtrl(const uint8_t* data, int width, int height, CWnd* pWnd)
{
    ...
    if (ctrlId == IDC_CAM_TOP) {
        flipH = m_bTopFlipH;  flipV = m_bTopFlipV;
    }
    else if (ctrlId == IDC_CAM_FRONT) {
        flipH = m_bSideFlipH; flipV = m_bSideFlipV;
    }


    // 2) 필요시 반전용 버퍼 준비
    const uint8_t* pDraw = data;
    std::vector<uint8_t> temp; // 반전 발생 시 여기에 출력
    if (flipH || flipV) {
        ...
        // StretchDIBits는 연속 버퍼가 필요
        temp.assign(dst.datastart, dst.dataend);
        pDraw = temp.data();
    }
    ...
    const double srcAR = (double)width / (double)height;
    const double dstAR = (double)rc.Width() / (double)rc.Height();
    int drawW, drawH, drawX, drawY;
    if (srcAR > dstAR) {
        drawW = rc.Width(); drawH = (int)(drawW / srcAR); drawX = 0; drawY = (rc.Height() - drawH) / 2;
    }
    else {
        drawH = rc.Height(); drawW = (int)(drawH * srcAR); drawX = (rc.Width() - drawW) / 2; drawY = 0;
    }
    ...
    StretchDIBits(dc.GetSafeHdc(), drawX, drawY, drawW, drawH,
        0, 0, width, height, pDraw, &bmi, DIB_RGB_COLORS, SRCCOPY);
    ...
    try {
        ...
        if (bUseTop && m_camTop.IsGrabbing()) {
            if (m_camTop.RetrieveResult(800, grabTop, TimeoutHandling_ThrowException) && grabTop->GrabSucceeded()) {
                ...
                if (m_bTopFlipH || m_bTopFlipV) {
                    int code = (m_bTopFlipH && m_bTopFlipV) ? -1 : (m_bTopFlipH ? 1 : 0);
                    cv::flip(frameTop, frameTop, code);
                }
        ...
        // 1) TOP 전송 후 잠깐 대기 (서버 동기화 호환)
        Sleep(200); // 이전 정상 코드와 동일한 딜레이(필요 시 150~250 조정)
        ...
        if (bUseSide && m_camSide.IsGrabbing()) {
            if (m_camSide.RetrieveResult(800, grabSide, TimeoutHandling_ThrowException) && grabSide->GrabSucceeded()) {
                ...
                if (m_bSideFlipH || m_bSideFlipV) {
                    int code = (m_bSideFlipH && m_bSideFlipV) ? -1 : (m_bSideFlipH ? 1 : 0);
                    cv::flip(frameSide, frameSide, code);
                }
                // ★이제 SIDE 응답도 받는다
                SendImageToServer(bufSide, _T("SIDE"), sideResponse); // 실패해도 진행
            }
        }

        // 3) 응답 파싱 우선순위: SIDE → TOP
        const std::string* pJson = nullptr;
        if (!sideResponse.empty()) pJson = &sideResponse;
        else if (!topResponse.empty()) pJson = &topResponse;

        // 4) 선택된 응답으로 파싱 시도
        if (pJson && ParseJsonResponse(*pJson, *pResult)) {
            if (GetSafeHwnd()) PostMessage(WM_APP_CAPTURE_COMPLETE, 1, (LPARAM)pResult);
        }
        else {
            // 파싱 실패: 원문 전달
            const std::string& raw = pJson ? *pJson : std::string();
            char* pStr = new char[raw.length() + 1];
            strcpy_s(pStr, raw.length() + 1, raw.c_str());
...
// --- Network Functions ---
// ========================================================================
// [REPLACE] SendImageToServer (반복 수신 + HTTP 헤더/공백 제거 + JSON 추출)
// ========================================================================
bool CCanClientDlg::SendImageToServer(const std::vector<unsigned char>& imgBuffer, CString role, std::string& response)
{
    ...
    if (inet_pton(AF_INET, CT2A(m_strServerIP), &serverAddr.sin_addr) != 1) {
        AddLog(L"[ERROR] inet_pton 실패 (업로드 서버 IP)");
        closesocket(sock);
        return false;
    }
    ...
    if (connect(sock, (sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
        ...
        CString errMsg;
        errMsg.Format(L"[ERROR] 업로드 서버(%s:%d) 연결 실패 (WSA: %d)",
            (LPCTSTR)m_strServerIP, m_nUploadPort, err);
        AddLog(errMsg);
        closesocket(sock);
        return false;
    }
    ...
    u_long nonblock = 1;
    ioctlsocket(sock, FIONBIO, &nonblock); // 논블로킹
    ...
    for (;;) {
        ...
        if (r > 0) {
            ...
            // (옵션) 본문이 다 들어온 듯하면 일찍 끝낼 수도 있음
            // if (raw.find('\n') != std::string::npos || raw.find('}') != std::string::npos) { /* keep waiting; rely on timeout */ }
        }
        else if (r == 0) {
            // 서버가 정리하고 닫음 -> 수신 종료
            break;
            ...
            if (e != WSAEWOULDBLOCK && e != WSAEINPROGRESS) {
                AddLog(L"[ERROR] recv 실패");
                break;
            }
            Sleep(10); // 잠깐 쉼
        }
        if (GetTickCount() - start > 2000) break; // 2초 타임아웃
    }
    ...
    if (raw.rfind("HTTP/", 0) == 0) {
        size_t p = raw.find("\r\n\r\n");
        if (p != std::string::npos) body = raw.substr(p + 4);
    }
    ...
    response = body;
    AddLog(L"[INFO] 서버 응답(정제): " + Utf8ToCStr(response));
...
void CCanClientDlg::OnDblclkListHistory(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    *pResult = 0;
    if (pNMItemActivate->iItem != -1 && pNMItemActivate->iItem < m_historyList.GetItemCount()) {
        ...
        if (bGotTop || bGotSide) {
            // 기존 창이 열려있으면 닫고 포인터 해제
            if (m_pPreviewDlg && ::IsWindow(m_pPreviewDlg->GetSafeHwnd())) {
                m_pPreviewDlg->DestroyWindow();
                // PostNcDestroy에서 delete 예정
                m_pPreviewDlg = nullptr;
            }

            // 새 모델리스 생성
            m_pPreviewDlg = new CPreviewDlg(this);
            m_pPreviewDlg->SetModeless(TRUE);
            m_pPreviewDlg->Create(IDD_PREVIEW_DLG, this);
            m_pPreviewDlg->SetImagePaths(bGotTop ? pathTop : _T(""), bGotSide ? pathSide : _T(""));
            // 항상 위 + 앞으로
            // 교체: "항상 위"이되 "포커스/활성화는 빼고" 표시 => 메인창 조작 유지
            m_pPreviewDlg->ModifyStyleEx(0, WS_EX_TOPMOST | WS_EX_TOOLWINDOW);
            m_pPreviewDlg->SetWindowPos(&CWnd::wndTopMost, 0, 0, 0, 0,
                SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOACTIVATE);
            m_pPreviewDlg->ShowWindow(SW_SHOWNOACTIVATE); // 포커스 훔치지 않음
            m_pPreviewDlg->RedrawWindow(nullptr, nullptr, RDW_ALLCHILDREN | RDW_ERASE | RDW_INVALIDATE);
...
bool CCanClientDlg::ParseJsonResponse(const std::string& jsonStr, InspectionResult& result)
{
    ...
    // 간단 trim
    while (!trimmed.empty() && (trimmed.front() == '"' || isspace((unsigned char)trimmed.front()))) trimmed.erase(trimmed.begin());
    while (!trimmed.empty() && (trimmed.back() == '"' || isspace((unsigned char)trimmed.back())))  trimmed.pop_back();
    if (!trimmed.empty() && trimmed.front() != '{' && trimmed.size() <= 12) {
        result.defectType = Utf8ToCStr(trimmed);
    ...
    try {
        ...
        // 키 이름 관용 처리: result / status / outcome / code
        CString defectType;
        if (j.contains("result"))          defectType = Utf8ToCStr(j["result"].get<std::string>());
        else if (j.contains("status"))     defectType = Utf8ToCStr(j["status"].get<std::string>());
        else if (j.contains("outcome"))    defectType = Utf8ToCStr(j["outcome"].get<std::string>());
        else if (j.contains("code"))       defectType = Utf8ToCStr(j["code"].get<std::string>());
        else {
            // 값만 있는 배열/객체인 경우 첫 문자열 필드 픽업
            for (auto& [k, v] : j.items()) {
                if (v.is_string()) { defectType = Utf8ToCStr(v.get<std::string>()); break; }
            }
        }

        CString detail;
        if (j.contains("reason"))          detail = Utf8ToCStr(j["reason"].get<std::string>());
        else if (j.contains("message"))    detail = Utf8ToCStr(j["message"].get<std::string>());
        else if (j.contains("detail"))     detail = Utf8ToCStr(j["detail"].get<std::string>());

        if (defectType.IsEmpty()) defectType = _T("파싱오류");
        if (detail == _T("N/A"))  detail.Empty();
        ...
        // 3) 최후 수단: 텍스트에서 의미 추출
        auto low = toLower(jsonStr);
        if (low.find("정상") != std::string::npos || low.find("ok") != std::string::npos || low.find("pass") != std::string::npos) {
            result.defectType = _T("정상");
            result.defectDetail = _T("");
            return true;
        }
        if (low.find("불량") != std::string::npos || low.find("ng") != std::string::npos || low.find("error") != std::string::npos || low.find("fail") != std::string::npos) {
            result.defectType = _T("에러");
            result.defectDetail = Utf8ToCStr(jsonStr.c_str());
            return true;
        }
        AddLog(L"[ERROR] JSON 파싱 완전 실패");
        return false;
    }
}
...
void CCanClientDlg::OnBnClickedBtnSettings()
{
    ...
    m_settingsDlg.m_bTopFlipH = m_bTopFlipH;
    m_settingsDlg.m_bTopFlipV = m_bTopFlipV;
    m_settingsDlg.m_bSideFlipH = m_bSideFlipH;
    m_settingsDlg.m_bSideFlipV = m_bSideFlipV;
    ...
    if (m_settingsDlg.DoModal() == IDOK)
    {
        ...
        m_bTopFlipH = m_settingsDlg.m_bTopFlipH;
        m_bTopFlipV = m_settingsDlg.m_bTopFlipV;
        m_bSideFlipH = m_settingsDlg.m_bSideFlipH;
        m_bSideFlipV = m_settingsDlg.m_bSideFlipV;
        ...
        CString f;
        f.Format(L"[INFO] Flip 설정: TOP(H=%d,V=%d), SIDE(H=%d,V=%d)",
            m_bTopFlipH, m_bTopFlipV, m_bSideFlipH, m_bSideFlipV);
        AddLog(f);
